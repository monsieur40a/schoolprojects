...
def optimize_hyperspace_routes(star_list,jump_times):
    n = len(jump_times) # get # of rows
    Pred =  [list(ls) for ls in jump_times]

    for i in range(n): #initialize prev matrix
        for j in range(n):
            if jump_times[i][j] == 0:
                Pred[i][j] = None
            elif jump_times[i][j] == float("inf"):
                Pred[i][j] = None
            else:
                Pred[i][j] = i
    ...
    for k in range(n):
        for i in range(n):
            for j in range(n):
                if jump_times[i][j] > jump_times[i][k] + jump_times[k][j]:
                    jump_times[i][j] = jump_times[i][k] + jump_times[k][j]
                    Pred[i][j] = Pred[k][j]

    result = helper(Pred, n)
    if result == None:
        return jump_times
    else:
        row = result
        first = Pred[row][Pred[row][row]]
        next = first
        A = [star_list[first]]
        firstpass = True
        while(first != next or firstpass):
            next = Pred[row][next]
            A.append(star_list[next])
            firstpass = False
        A.reverse()
        return A

def helper(PredMat, n):
    for i in range(n):
        if PredMat[i][i] != None:
            return i
    return None
