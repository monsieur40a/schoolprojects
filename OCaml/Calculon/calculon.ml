...
let rec process_statement inchan =
  ...
  begin
    try
      ...
      (* let tokens = Calclex.lex_stream input in
      let toks = Stream.next tokens in *)
      (* let tokens = List.filter (fun x -> match x with | None -> false | Some x -> true) tokens in
      let tokens = List.map (fun x ->
        match x with | Some x -> x | _ -> failwith "wtf mate") tokens in *)
